---
title: "Regridded to COMBO 2.5 deg grid, GFDL-CM4_ssp245 annual, local tos to global tas pattern scaling"
author: "ACS"
date: "12/19/2019"
output:
  html_document:
...
```{r names}
esmname <- 'rgr2p5-GFDL-CM4_ssp245'
...
# #############################################################################
## for COMBO, tos values actually have to be in change from 2007 tos, in degC
# #############################################################################
...
rowindex2007 <- which(histyears == 2007)


# load the actual netcdf that has the 2007 data
tos2007grid <- lapply(tos2007filename, readtos)


# Check the units on 2007 tos, convert to degC if necessary:
  ncin <- nc_open(tos2007filename)
  tosunit <-  ncatt_get(ncin, 'tos', 'units')$value
  nc_close(ncin)
      
  print(paste(tos2007filename,'unit is', tosunit))
      
  if(tosunit == 'K'){
    
    print('converting from K to degC')
    tos2007grid$vardata <- (tos2007grid$vardata - 273.15)
...
tos2007grid <- concatGrids.general(tos2007grid)  


# Convert the data containing 2007 from monthly data to annual
...
tos2007_annual_grid <- tos2007_annual_grid[rowindex2007,]

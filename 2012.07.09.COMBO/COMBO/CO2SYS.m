...
% Input conditioning - make sure each vector is a COLUMN vector:
if diff(size(PAR1))>0;          PAR1=PAR1'; end;
if diff(size(PAR2))>0;          PAR2=PAR2'; end;
if diff(size(PAR1TYPE))>0;      PAR1TYPE=PAR1TYPE'; end;
if diff(size(PAR2TYPE))>0;      PAR2TYPE=PAR2TYPE'; end;
if diff(size(SAL))>0;           SAL=SAL'; end;
if diff(size(TEMPIN))>0;        TEMPIN=TEMPIN'; end;
if diff(size(TEMPOUT))>0;       TEMPOUT=TEMPOUT'; end;
if diff(size(PRESIN))>0;        PRESIN=PRESIN'; end;
if diff(size(PRESOUT))>0;       PRESOUT=PRESOUT'; end;
if diff(size(PO4))>0;           PO4=PO4'; end;
if diff(size(SI))>0;            SI=SI'; end;
if diff(size(pHSCALEIN))>0;     pHSCALEIN=pHSCALEIN'; end;
if diff(size(K1K2CONSTANTS))>0; K1K2CONSTANTS=K1K2CONSTANTS'; end;
if diff(size(KSO4CONSTANTS))>0; KSO4CONSTANTS=KSO4CONSTANTS'; end;

ntps = max([length(PAR1) length(PAR2) length(SAL) length(TEMPIN) length(TEMPOUT) length(PRESIN) length(PRESOUT) length(SI) length(PO4) length(pHSCALEIN) length(K1K2CONSTANTS) length(KSO4CONSTANTS)]);
if ntps>1
    if isscalar(PAR1)          ; PAR1(1:ntps,1)          = PAR1          ; end
    if isscalar(PAR2)          ; PAR2(1:ntps,1)          = PAR2          ; end
    if isscalar(PAR1TYPE)      ; PAR1TYPE(1:ntps,1)      = PAR1TYPE      ; end
    if isscalar(PAR2TYPE)      ; PAR2TYPE(1:ntps,1)      = PAR2TYPE      ; end
    if isscalar(SAL)           ; SAL(1:ntps,1)           = SAL           ; end
    if isscalar(TEMPIN)        ; TEMPIN(1:ntps,1)        = TEMPIN        ; end
    if isscalar(TEMPOUT)       ; TEMPOUT(1:ntps,1)       = TEMPOUT       ; end
    if isscalar(PRESIN)        ; PRESIN(1:ntps,1)        = PRESIN        ; end
    if isscalar(PRESOUT)       ; PRESOUT(1:ntps,1)       = PRESOUT       ; end
    if isscalar(SI)            ; SI(1:ntps,1)            = SI            ; end
    if isscalar(PO4)           ; PO4(1:ntps,1)           = PO4           ; end
    if isscalar(pHSCALEIN)     ; pHSCALEIN(1:ntps,1)     = pHSCALEIN     ; end
    if isscalar(K1K2CONSTANTS) ; K1K2CONSTANTS(1:ntps,1) = K1K2CONSTANTS ; end
    if isscalar(KSO4CONSTANTS) ; KSO4CONSTANTS(1:ntps,1) = KSO4CONSTANTS ; end
end
...
%F=find(p1==5 | p2==5); PC(F) = FC(F)./FugFac(F);
...
DATA=[TAc*1e6        TCc*1e6         PHic           PCic*1e6        FCic*1e6...
      ...
      PAlkinp*1e6    SiAlkinp*1e6    Hfreeinp       Revelleinp      OmegaCainp...
      OmegaArinp     xCO2dryinp*1e6  PHoc           PCoc*1e6        FCoc*1e6...
      HCO3out*1e6    CO3out*1e6      CO2out*1e6     BAlkout*1e6     OHout*1e6...
      PAlkout*1e6    SiAlkout*1e6    Hfreeout       Revelleout      OmegaCaout...
...
if any(F)
    TB(F) = (0.000232./10.811).*(Sal(F)./1.80655); % in mol/kg-SW
...
if any(F)
    ...
    logK1(F) = 13.7201 - 0.031334.*TempK(F) - 3235.76./TempK(F)...
        - 0.000013.*Sal(F).*TempK(F) + 0.1032.*sqrSal(F);
    K1(F) = 10.^(logK1(F))...         % this is on the NBS scale
        ./fH(F);                   % convert to SWS scale
    %
    logK2(F) = -5371.9645 - 1.671221.*TempK(F) + 128375.28./TempK(F)...
        + 2194.3055.*logTempK(F)./log(10) - 0.22913.*Sal(F)...
        - 18.3802.*log(Sal(F))./log(10) + 0.00080944.*Sal(F).*TempK(F)...
        + 5617.11.*log(Sal(F))./log(10)./TempK(F) - 2.136.*Sal(F)./TempK(F);
    K2(F) = 10.^(logK2(F))...         % this is on the NBS scale
        ./fH(F);                   % convert to SWS scale
end
F=(WhichKs==8);
if any(F)
...
F=(WhichKs==9);
if any(F)
	% Below removed by SvH20090527, after suggestion by Denis Pierrot. Have been superceded
	% From Millero 2005 work on pK1 and pK2 from titrations
	% pK1(F) = -(290.9097 - 14554.21./TempK(F) - 45.0575.*log(TempK(F)))./log(10) + 13.9041.*...
	%    sqrt(Sal(F)) + 0.03494.*Sal(F) - 0.0000495.*Sal(F).^2 - sqrt(Sal(F)).*(548.1768./...
	%    TempK(F) + 2.143796.*log(TempK(F))) - 6.7347.*Sal(F)./TempK(F);
    % pK2(F) = -(207.6548 - 11843.79./TempK(F) - 33.6485.*log(TempK(F)))./log(10) + 20.9686.*...
	...
	%      6.103.*Sal(F)./TempK(F); %    Added SvH20090527, suggested by Denis Pierrot		
     ...
     pK1(F) =  -126.34048 + 6320.813./TempK(F) + 19.568224.*log(TempK(F)) ...
	 			...
	 			+ (-530.123 .*sqrt(Sal(F)) - 6.103.*Sal(F)) ./ TempK(F) ...
    ...
    pK2(F) =  -90.18333 + 5143.692./TempK(F) + 14.613358.*log(TempK(F)) ...
	 			...
	 			+ (-772.483 .*sqrt(Sal(F)) - 20.051.*Sal(F)) ./ TempK(F) ...
	 			+ (  -3.3336.*sqrt(Sal(F))) .* log(TempK(F)); %    Added SvH20090527, suggested by Denis Pierrot		this is better...
	K1(F)  = 10.^(-pK1(F));
    K2(F)  = 10.^(-pK2(F));

 	% pK2 = -90.18333 + 5143.692 ./ TempK + 14.613358 .* log(TempK) + 21.0894 .* sqrt(Sal) + 0.1248 .* Sal - 0.0003687 .* Sal .^ 2 - sqrt(Sal) .* (772.483 ./ TempK + 3.3336 * log(TempK)) - 20.051 .* Sal ./ TempK;
    %  K2 = 10 .^ (-pK2);
...
if any(F)
    ...
    K1(F) = 10.^-( 3633.86./TempK(F) - 61.2172 + 9.67770.*log(TempK(F)) - 0.011555.*Sal(F) + 0.0001152.*Sal(F).^2 );
    K2(F) = 10.^-(  471.78./TempK(F) + 25.9290 - 3.16967.*log(TempK(F)) - 0.01781 .*Sal(F) + 0.0001122.*Sal(F).^2 );
end
...
global FugFac F;
% Outputs pH pCO2 fCO2, in that order
...
% Outputs: pH, pCO2, fCO2
% This calculates pH, pCO2, and fCO2 from TA and TC at output conditions.
...
TSF=TS(F);   KSF=KS(F);   TFF=TF(F);   KFF=KF(F);
H         = 10.^(-pHx);
HCO3      = TCx.*K1F.*H  ./(K1F.*H + H.*H + K1F.*K2F);
CO3       = TCx.*K1F.*K2F./(K1F.*H + H.*H + K1F.*K2F);
BAlk      = TBF.*KBF./(KBF + H);
OH        = KWF./H;
PhosTop   = KP1F.*KP2F.*H + 2.*KP1F.*KP2F.*KP3F - H.*H.*H;
PhosBot   = H.*H.*H + KP1F.*H.*H + KP1F.*KP2F.*H + KP1F.*KP2F.*KP3F;
...
varargout{1} = CO3.*CA./KCa;% 'dimensionless
varargout{2} = CO3.*CA./KAr;% 'dimensionless
end % end nested function
...
varargout{1}=pHtot;   varargout{2}=pHsws;
varargout{3}=pHfree;  varargout{4}=pHNBS;
end % end nested function